...
public class AppsFlyerPlugin: CAPPlugin {
    private let APPSFLYER_PLUGIN_VERSION = "6.10.2-rc1"
...
extension AppsFlyerPlugin{
    ...
    private func getPluginVersion2() -> String {
        let plist = readPlist(name: "AppsFlyerCap")
        let version =  plist["AppsFlyerPluginVersion"] as? String
        return version ?? "err"
    }
    
    private func readPlist2(name: String) -> [String: Any] {
    ...
    private func readPlist(name: String) -> NSDictionary {
        // Get the path to the plist file in your project
        if let plistPath = Bundle.main.path(forResource: "AppsflyerCapacitorPlugin-Info", ofType: "plist"){
            // Load the contents of the plist file into a Data object
        ...
        guard let plistPath = Bundle(for: AppsFlyerPlugin.self).path(forResource: "AppsflyerCapacitorPlugin-Info", ofType: "plist")else {
            return [:]
        }
    ...
    private func getPluginVersion() -> String {
        if let versionNumber = ProcessInfo.processInfo.environment["APPSFLYER_PLUGIN_VERSION"] {
            print("APPSFLYER_PLUGIN_VERSION: \(versionNumber)")
        } else {
            print("APPSFLYER_PLUGIN_VERSION not defined")
        }

      return  Bundle(for: AppsFlyerPlugin.self).infoDictionary?["APPSFLYER_PLUGIN_VERSION"] as? String ?? "failed_to_fetch"
