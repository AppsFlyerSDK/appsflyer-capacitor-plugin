---
name: rc-release
description: Describes how the RC release pipeline works for the AppsFlyer Capacitor plugin and maps each stage to its workflow file. Use when inspecting, triggering, or debugging the RC-release, rc-smoke, promote-release, or production-release workflows; not for day-to-day plugin development.
globs: .github/workflows/rc-release.yml,.github/workflows/rc-smoke.yml,.github/workflows/promote-release.yml,.github/workflows/production-release.yml,.github/workflows/lint-test-build.yml,.github/workflows/ios-e2e.yml,.github/workflows/android-e2e.yml,.af-e2e/**,.af-smoke/**,scripts/af-scenario-runner.sh,scripts/synthesize-rc-smoke-app.sh
---
...
- You're editing any of the four release workflows (`rc-release.yml`, `rc-smoke.yml`, `promote-release.yml`, `production-release.yml`).
...
| `RC-PUBLISH` | [`rc-release.yml`](../../../.github/workflows/rc-release.yml) | `publish-rc` (honours `dry_run`) |
...
| `RC-RELEASE` | [`production-release.yml`](../../../.github/workflows/production-release.yml) | `pull_request: closed` (merged) on `main` |
...
| `CI_DEV_GITHUB_TOKEN` | `RC-PREP`, `RC-PROMOTE`, `RC-SMOKE`, `RC-RELEASE` | branch push, PR ops, check_run posting |
...
Both `RC-PUBLISH` and `RC-RELEASE` use npm trusted publishing (`permissions: id-token: write` + `npm publish --provenance`). The OIDC JWT is exchanged for a per-job npm publish token; no `NPM_TOKEN` secret is stored. Trusted publishers must be configured on npmjs.com for both `rc-release.yml` and `production-release.yml`.
