...
- [Log Event](#logevent)
- [DeepLinking](#deeplinking) — see [`DeepLink.md`](DeepLink.md)
- [Uninstall](#uninstall)
...
Note: `init()` no longer has a built-in `waitForATTUserAuthorization` option — request the ATT
permission yourself, inside the session-ready callback, before calling `start()`, as shown above.
...
See [`DeepLink.md`](DeepLink.md) for deferred and unified deep linking setup, code samples, and
the Android/iOS manifest configuration.
